...
**Output file:** `cv/Brad Burch Resume - <Company>.tex`
...
```bash
cd cv && lualatex -interaction=nonstopmode "Brad Burch Resume - <Company>.tex"
```

Expected output: `Output written on Brad Burch Resume - <Company>.pdf (2 pages, ...)`. Any page count other than 2 is a failure that must be fixed before presenting to the user.
...
1. Run `lualatex -interaction=nonstopmode "Brad Burch Resume - <Company>.tex"`
...
```bash
cd cv && pdftotext -layout "Brad Burch Resume - <Company>.pdf" "Brad Burch Resume - <Company>.txt"
```
