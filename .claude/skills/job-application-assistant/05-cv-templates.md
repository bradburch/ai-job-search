...
**Output file:** `cv/Brad Burch Resume - <Company> - <Role>.tex`
...
Expected output: `Output written on Brad Burch Resume - <Company> - <Role>.pdf (2 pages, ...)`. Any page count other than 2 is a failure that must be fixed before presenting to the user.
...
**For Forward Deployed Engineer roles:**
> Forward Deployed Engineer who scopes the problem, builds the system, and owns it in production. 7 years across edge infrastructure, distributed services, and enterprise SaaS, most recently building and operating a production Claude agent and an OAuth 2.1 + PKCE MCP server for a live business with paying customers. Was the engineering point of contact on enterprise Customer Success and Sales calls at Total Brain; still on customer calls weekly for my own product.
...
*[Used for: general FDE / SWE applications - adapted from prior resume drafts, verified against the candidate profile]*
...
1. Run `lualatex -interaction=nonstopmode "Brad Burch Resume - <Company> - <Role>.tex"`
